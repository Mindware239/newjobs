    ...
    public function register(Request $request, Response $response): void
    {
        ...
        if ($user->save()) {
            ...
            if ($data['role'] === 'employer') {
                ...
                $candidate = $service->ensureCandidateForUser((int)$user->id, array_merge($candidateData, [
    ...
    public function registerEmployer(Request $request, Response $response): void
    {
        ...
        try {
            ...
            $settings->fill([
                ...
                'billing_plan' => 'free',
                'credits' => 0,
                'timezone' => (isset($data['country']) && $data['country'] === 'India') ? 'Asia/Kolkata' : 'UTC'
            ]);
            if (!$settings->save()) {
    ...
    public function registerCandidate(Request $request, Response $response): void
    {
        ...
        try {
            ...
            try {
                ...
                $admins = $db->fetchAll("SELECT email FROM users WHERE role IN ('admin','masteradmin') AND status = 'active' AND email IS NOT NULL");
                $candName = (string)($candidate->attributes['full_name'] ?? ($candidateData['full_name'] ?? 'Candidate'));
        ...
        if ($user->role === 'employer') {
            ...
            if (!$candidate) {
                ...
                $candidate = $service->ensureCandidateForUser((int)$user->id, [
                    ...
                    'is_profile_complete' => 0,
                    'created_by' => 'oauth',
                    'source' => ($_SESSION['oauth_provider'] ?? 'oauth')
                ]);
            }
