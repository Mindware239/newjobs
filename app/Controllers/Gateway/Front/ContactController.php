...
class ContactController
{
    ...
    public static function submitForm(Request $request): void
    {
        ...
        // Use NotificationService to send via SMTP (centralized logging)
        try {
            // We use queueEmail directly since we don't have a user ID for the visitor
            // and we are sending TO the admin (configured in env or settings)
            \App\Services\NotificationService::queueEmail(
                $to, 
                'generic_notification', // Use generic template
                [
                    'title' => $email_subject,
                    'message' => "From: {$name} ({$email})<br><br>" . nl2br($message),
                    'link' => $_ENV['APP_URL'] ?? '/',
