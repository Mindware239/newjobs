...
<head>
    ...
    <style>
        ...
        .bg-blue-50, .hover\:bg-blue-50:hover, .bg-indigo-50, .bg-purple-50 {
            background-color: var(--color-active-menu-bg) !important
        }

        .text-blue-700, .text-blue-600, .text-indigo-700, .text-purple-700 {
        ...
        .hover\:bg-orange-700:hover, .hover\:bg-orange-600:hover {
            background-color: var(--color-primary-hover) !important
        }
        ...
        .bg-orange-100, .bg-orange-200 {
            background-color: var(--color-active-menu_bg, var(--color-active_menu_bg));
            background-color: var(--color-active-menu-bg) !important
        }
        ...
        .hover\:text-orange-600:hover, .hover\:text-green-700:hover {
            color: var(--color-primary) !important
        }
    ...
    <nav class="bg-white border-b sticky top-0 z-50" x-data="{ techOpen:false, mobileOpen:false, mobileTech:false }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="hidden md:flex items-center gap-6 text-sm font-semibold">
                    <a href="<?= htmlspecialchars($topLinks['Home'] ?? '/blog') ?>" class="text-gray-800 hover:text-green-700">HOME</a>
                    <a href="<?= htmlspecialchars($topLinks['Career Advice'] ?? '/blog') ?>" class="text-gray-800 hover:text-green-700">CAREER
                        ADVICE</a>
                    <div class="relative">
                        <button @click="techOpen=!techOpen" @click.outside="techOpen=false"
                                class="flex items-center gap-1 text-gray-800 hover:text-green-700">
                            TECHNOLOGY
                        ...
                        <div x-show="techOpen"
                             class="absolute left-0 mt-3 w-64 bg-white shadow-xl border rounded-xl z-50 p-2">
                            <?php foreach ($techSubs as $name): ?>
                                <?php $lower = strtolower($name); ?>
                                <?php if (!empty($catByName[$lower])): ?>
                                    <a href="/blog/category/<?= htmlspecialchars($catByName[$lower]['slug']) ?>"
                                       class="block px-3 py-2 text-gray-800 hover:text-green-700">
                    ...
                    <a href="<?= $topLinks['Interview Questions'] ?>" class="text-gray-800 hover:text-green-700">INTERVIEW
                        QUESTIONS</a>
                    ...
                    <a href="<?= $topLinks['Interview Advice'] ?>" class="text-gray-800 hover:text-green-700">INTERVIEW
                        ADVICE</a>
                </div>
            </div>
        </div>
        <div class="md:hidden border-t" x-show="mobileOpen" x-transition>
            <div class="px-4 py-3 space-y-2 text-sm font-semibold">
                ...
                <a href="<?= $topLinks['Career Advice'] ?>" class="block px-2 py-2 text-gray-800 hover:text-green-700">CAREER
                    ADVICE</a>
                ...
                <div x-show="mobileTech" x-transition class="ml-2 space-y-1">
                    <?php foreach ($techSubs as $name): ?>
                        ...
                        <?php if (!empty($catByName[$lower])): ?>
                            <a href="/blog/category/<?= htmlspecialchars($catByName[$lower]['slug']) ?>"
                               class="block px-2 py-1 text-gray-700 hover:text-green-700"><?= htmlspecialchars($name) ?></a>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </div>
                <a href="<?= $topLinks['Interview Questions'] ?>"
                   class="block px-2 py-2 text-gray-800 hover:text-green-700">INTERVIEW QUESTIONS</a>
                ...
                <a href="<?= $topLinks['Interview Advice'] ?>"
                   class="block px-2 py-2 text-gray-800 hover:text-green-700">INTERVIEW ADVICE</a>
            </div>
        </div>
    </nav>
<?php else: ?>
    <?php $GLOBALS['layout_header'] = 1;
                    ...
                    <ul class="space-y-2">
                        <?php if (!empty($categories)): ?>
                            <?php foreach ($categories as $c): ?>
                                <li>
                                    <a class="text-gray-700 hover:text-green-700"
                                       href="/blog/category/<?= htmlspecialchars($c['slug'] ?? '') ?>">
                    ...
                    <ul class="space-y-2">
                        <?php if (!empty($latestArticles)): ?>
                            <?php foreach ($latestArticles as $la): ?>
                                <li>
                                    <a class="text-gray-700 hover:text-green-700"
                                       href="/blog/<?= htmlspecialchars($la['slug'] ?? '') ?>">
