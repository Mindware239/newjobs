...
<div class="home-wide">
    <section class="relative hero-height bg-white text-gray-800">
        <div class="relative container mx-auto px-4 lg:px-8 pt-16 md:pt-20 pb-6">
            ...
            <h2 class="text-4xl md:text-6xl font-extrabold tracking-tight text-gray-900 text-center leading-tight">
                Handpicked Premium
                <span id="hero-word" class="text-blue-600"></span>
                <span id="hero-caret" class="text-blue-600 blink">|</span>
                Jobs
            </h2>
            <script>
                (function () {
            ...
            <div class="mt-10 max-w-6xl mx-auto px-4">
                ...
                <div class="hidden md:flex items-center bg-white rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 p-2 max-w-5xl mx-auto ring-1 ring-gray-50">
                    ...
                    <div class="flex items-center flex-1 px-4 relative group" x-data="homeKeywordSuggest()">
                        <svg class="w-5 h-5 text-gray-400 group-hover:text-blue-500 transition-colors mr-3 shrink-0"
                             fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                        </svg>
                        ...
                        <div x-show="show && list.length>0" x-cloak
                            ...
                            <template x-for="(s, i) in list" :key="s.id ?? i">
                                <div @click="select(s)" @mouseenter="selectedIndex=i"
                                     class="px-5 py-3 cursor-pointer hover:bg-blue-50/50 transition-colors flex items-center gap-3"
                                     :class="selectedIndex===i ? 'bg-blue-50/50' : ''">
                    ...
                    <div x-data="locationDropdown()" class="relative min-w-[240px] px-6 group">
                        <button type="button" @click="open = !open"
                            ...
                            <div class="flex items-center gap-3 truncate">
                                <svg class="w-5 h-5 text-gray-400 group-hover:text-blue-500 transition-colors shrink-0"
                                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <template x-for="item in filteredItems()" :key="item.value">
                                    <li @click="selectItem(item)"
                                        ...
                                        <span x-text="item.label"
                                              class="text-sm text-gray-700 group-hover/item:text-blue-700 font-medium"></span>
                                        <span x-show="item.value === selectedValue" class="text-blue-600">
                    ...
                    <div x-data="{
                        ...
                        <button type="button" @click="open = !open"
                            ...
                            <svg class="w-4 h-4 text-gray-300 group-hover:text-blue-500 transition-transform"
                                 :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor"
                                 viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                      d="M19 9l-7 7-7-7"/>
                        ...
                        <div x-show="open" x-transition.opacity.duration.200ms @click.away="open = false"
                            ...
                            <div class="p-3 border-b border-gray-100">
                                <input type="text" x-model="search" placeholder="Filter experience…"
                                       class="w-full px-4 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            ...
                            <div class="max-h-64 overflow-y-auto py-1 custom-scrollbar">
                                <template x-for="opt in filtered" :key="opt">
                                    <div @click="selected = opt; open = false; search = ''"
                                         class="px-5 py-2.5 cursor-pointer text-sm font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors flex justify-between items-center">
                                        <span x-text="opt"></span>

                                        <svg x-show="selected === opt" class="w-4 h-4 text-blue-600" fill="none"
            ...
            <div class="mt-14 bg-white py-6 shadow-sm border-t border-b border-gray-100 relative" x-data="{
                ...
                <div class="container mx-auto px-6 lg:px-[7.5rem] relative group">
                    <button type="button"
                            class="absolute left-4 lg:left-20 top-1/2 -translate-y-1/2 p-2 text-gray-300 hover:text-gray-800 transition"
                    ...
                    <div id="cat-scroll"
                         class="flex items-center justify-between gap-8 overflow-x-auto no-scrollbar scroll-smooth px-12 py-1">
                        <?php if (!empty($categories) && is_array($categories)): ?>
                            <?php foreach ($categories as $index => $cat): ?>
                                <a href="<?= $base ?>jobs?industry=<?= urlencode($cat['slug'] ?? $cat['name'] ?? '') ?>"
                        ...
                        <?php else: ?>
                            ...
                            foreach ($demoCats as $cat):
                                ?>
                                <a href="#"
                                   class="group/cat flex flex-col items-center gap-4 min-w-[120px] cursor-pointer py-2">
                    ...
                    <button type="button"
                            class="absolute right-4 lg:right-20 top-1/2 -translate-y-1/2 p-2 text-blue-600 hover:text-blue-700 transition"
                            onclick="document.getElementById('cat-scroll').scrollBy({left: 200, behavior: 'smooth'})">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-data="{
                    ...
                    <div class="lg:w-1/4 flex-shrink-0 flex flex-col justify-center">
                        ...
                        <h2 class="text-3xl md:text-4xl text-gray-900 leading-[1.2] tracking-tight">
                            ...
                            <span id="tw-caret" class="text-blue-600 blink">|</span>
                        ...
                        <div class="flex items-center gap-4 mt-8">
                            <button type="button" @click="prev()"
                                    :class="{'opacity-30 cursor-not-allowed': active <= 0, 'hover:text-blue-700': active > 0}"
                            ...
                            <button type="button" @click="next()"
                                    :class="{'opacity-30 cursor-not-allowed': active >= max, 'hover:text-blue-700': active < max}"
                                       ...
                                       class="block h-full">
                                        <div class="h-full rounded-xl overflow-hidden bg-white border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 group/card">
                                            ...
                                            <div class="h-48 bg-gray-100 relative">
                                                ...
                                                <img loading="lazy"
                                                     src="https://images.unsplash.com/photo-<?= $unsplashImgs[$index % count($unsplashImgs)] ?>?auto=format&fit=crop&w=500&q=80"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
                ...
                <div>
                    ...
                    <div class="mt-6 flex items-center gap-3">
                        <a href="<?= $base ?>jobs"
                           class="px-6 py-3 rounded-full bg-blue-600 hover:bg-blue-700 hover:text-white text-white font-semibold">Search
                            Job</a>
                        <a href="<?= $base ?>jobs"
                           class="px-6 py-3 rounded-full bg-blue-50 text-blue-600 font-semibold">Learn more</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container mx-auto px-6 lg:px-[7.5rem]">
            <div class="rounded-3xl overflow-hidden bg-black relative min-h-[320px] md:min-h-[420px]">
                ...
                <div class="relative px-8 py-14 md:px-16 md:py-16 lg:pl-20 text-white max-w-xl md:ml-6 lg:ml-10 xl:ml-14">
                    <h3 class="text-3xl md:text-4xl font-semibold text-blue-600">Find Real Jobs <br>From Verified
                        Employers</h3>
                    ...
                    <a href="<?= $base ?>register-candidate"
                       class="mt-6 inline-block px-6 py-3 rounded-full bg-blue-600 hover:bg-blue-700 hover:text-white text-white font-semibold">Get
                        Started — It’s Free</a>
                </div>
            </div>
        </div>
                            ...
                            <div class="mt-4 flex items-center gap-3" x-show="items.length > 1">
                                ...
                                <div class="ml-2 flex items-center gap-1">
                                    <template x-for="(d,i) in items" :key="i">
                                        <span @click="go(i)" :class="i===active ? 'w-6 bg-blue-600' : 'w-3 bg-blue-200'"
    ...
    <section class="relative py-16 bg-white overflow-hidden">
        <div class="absolute -top-4 right-0 pointer-events-none select-none">
            <img loading="lazy" src="<?= $base ?>assets/images/bg-arrow.svg" alt="" class="w-24 h-24 opacity-40"/>
        </div>
                                ...
                                <div class="flex-shrink-0">
                                    <?php if (!empty($logo)): ?>
                                        <img loading="lazy" src="<?= htmlspecialchars($logo) ?>"
                                             alt="<?= htmlspecialchars($emp['company_name'] ?? '') ?>"
                                             class="h-12 object-contain partners-logo transition duration-300 ease-out hover:scale-105"/>
                                    <?php else: ?>
    ...
    <section class="py-16 bg-white">
        <div class="container mx-auto px-6">
            <div class="flex items-center justify-between">
                ...
                <a href="<?= $base ?>blog"
                   class="hidden md:inline-block text-blue-600 font-semibold hover:underline whitespace-nowrap">View
                    all</a>
            </div>
