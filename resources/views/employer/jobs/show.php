...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900"><?php echo htmlspecialchars($job['title'] ?? 'Job Details') ?></h1>
            <p class="text-sm text-gray-600 mt-1">Job ID: <?php echo $job['id'] ?? 'N/A' ?></p>
        </div>
        <div class="flex items-center gap-2">
            <a href="/employer/jobs/<?php echo htmlspecialchars($job['slug'] ?? $job['id'] ?? '') ?>/edit"
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        ...
        <div class="p-4 sm:p-6 space-y-6">
            <div>
                ...
                <p class="text-base text-gray-900 font-semibold"><?php echo htmlspecialchars($job['title'] ?? 'N/A') ?></p>
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Description</label>
                <div class="mt-2 prose prose-sm max-w-none text-gray-700">
                    <?php echo $job['description'] ?? 'N/A' ?>
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div class="bg-gray-50 rounded-lg p-4">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Job Type</label>
                    <p class="text-base text-gray-900 font-semibold"><?php echo ucfirst(str_replace('_', ' ', $job['title'] ?? 'N/A')) ?></p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4">
...
<p class="text-base text-gray-900 font-semibold">
    <?php echo isset($job['min_experience'], $job['max_experience'])
    ? htmlspecialchars($job['min_experience']) .' Years - ' .htmlspecialchars($job['max_experience']) . ' Years'
    : 'N/A'
?>
</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <p class="text-base text-gray-900 font-semibold">
                        <?php echo ($job['salary_min'] ?? 0) > 0 ? number_format($job['salary_min']) : 'N/A' ?> -
                        <?php echo ($job['salary_max'] ?? 0) > 0 ? number_format($job['salary_max']) : 'N/A' ?>
                        <span class="text-sm text-gray-600"><?php echo $job['salary_currency'] ?? 'INR' ?></span>
                    </p>
                </div>
                <div class="bg-gray-50 rounded-lg p-4">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Location</label>
                    <p class="text-base text-gray-900 font-semibold"><?php echo htmlspecialchars($job['job_address'] ?? 'N/A') ?></p>
                </div>
            </div>
            <?php if (! empty($skills)): ?>
                <div>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <p class="text-base text-gray-900 font-semibold"><?php echo date('M d, Y', strtotime($job['created_at'] ?? 'now')) ?></p>
            </div>
        </div>
    </div>
                        ...
                        <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-purple-200 transition-all">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 mt-1">Status:
                                    <span class="font-semibold <?php echo ($application['status'] ?? '') === 'shortlisted' ? 'text-green-600' : (($application['status'] ?? '') === 'rejected' ? 'text-red-600' : 'text-gray-600') ?>">
