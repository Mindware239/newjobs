...
<div class="max-w-6xl mx-auto px-4 py-8">
  <h1 class="text-2xl font-bold mb-6">#<?= htmlspecialchars($tag['name'] ?? 'Tag') ?></h1>
  <div class="space-y-4">
    <?php if (!empty($blogs)): ?>
      ...
      <?php endforeach; ?>
    <?php else: ?>
      <div class="text-gray-500">No posts found for this tag</div>
    <?php endif; ?>
  </div>
  <?php
  ...
  <div class="flex items-center gap-2 mt-4">
    <?php if ($page > 1): ?>
      <a class="px-3 py-2 border rounded" href="/blog/tag/<?= htmlspecialchars($tag['slug'] ?? '') ?>?page=<?= $page - 1 ?>">Prev</a>
    <?php endif; ?>
    <span class="text-gray-600">Page <?= $page ?> of <?= $pages ?></span>
    <?php if ($page < $pages): ?>
      <a class="px-3 py-2 border rounded" href="/blog/tag/<?= htmlspecialchars($tag['slug'] ?? '') ?>?page=<?= $page + 1 ?>">Next</a>
    <?php endif; ?>
  </div>
</div>
